...
| 2   | [Configurer un Codespace](02a-setup-codespace.md)                                                          |
...
- [Configurer votre terminal local](side-quest-02-01-local-terminal.md) — alternative facultative pour les personnes à l’aise avec le terminal qui veulent travailler sur leur propre machine plutôt que dans le Codespace recommandé.
...
- [Use `gh-aw` with the GitHub Copilot Cloud Agent](side-quest-06-02-cca-codespace.md) — explique comment ouvrir un terminal dans le Codespace du Copilot Cloud Agent pour que les commandes CLI `gh-aw` fonctionnent ; dérive de [Step 6](06-install-gh-aw.md).
...
- [Fix Codespaces `actions:write` Errors When Running `gh aw run`](side-quest-08-01-codespaces-actions-write.md) — guide de dépannage pour les erreurs de permission de déclenchement de workflow dans Codespaces, avec une option via l’onglet Actions et des étapes avancées de récupération ; dérive de [Step 8](08-run-your-workflow.md).
